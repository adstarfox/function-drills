...
let nameGreeting = nameCheck(`Maddi`)
...
let colorRating = faveColorFinder(`ReD`.toLowerCase())
...
//CODE HERE
function theEliminator(contestant, losers){
  let newContestants = []
  for (let i = 0; i < contestant.length; i++){
    if (contestant[i].includes(losers)){
      contestant.splice()
    }else {
      newContestants.push(contestant[i])
    }
  }
...
const goldForFrogs = (gold) => {
  let frogsCost = 3
  let purchase = parseInt(gold) / frogsCost
  return purchase
}

let totalFrogs = console.log(goldForFrogs(7))
...
//CODE HERE
const goldForFrogs2 = (gold) => {
  let frogsCost = 3
  let purchase = Math.floor(parseInt(gold) / frogsCost)
  return purchase
}

let totalFrogs2 = console.log(goldForFrogs2(7))


////////////////// PROBLEM 21 ////////////////////
let sampleArray = [0,1,2,3,7,5,8,9]
/*
...
function takesArray (array){
    for (let i = 0; i < array.length && ascension === true; i++){
...
let arrayIsAscending = console.log(takesArray(sampleArray))
